...
import { Printer, Package, Truck, Calendar, User, Hash, Weight, Box, DollarSign } from 'lucide-react';
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800">
                  <th className="px-3 py-2 text-left text-xs font-semibold text-slate-600 dark:text-slate-400">№ п/п</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-slate-600 dark:text-slate-400">Номер заказа</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-slate-600 dark:text-slate-400">Мест</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-slate-600 dark:text-slate-400">Вес, кг</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-slate-600 dark:text-slate-400">Объем, м³</th>
                  <th className="px-3 py-2 text-right text-xs font-semibold text-slate-600 dark:text-slate-400">Прибыль, ₽</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {consolidation.orders.map((order, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <td className="px-3 py-2 text-sm text-slate-600 dark:text-slate-400">{idx + 1}</td>
                    <td className="px-3 py-2 text-sm font-medium text-slate-900 dark:text-white">{order.orderNumber}</td>
                    <td className="px-3 py-2 text-sm text-slate-600 dark:text-slate-400">{order.quantity}</td>
                    <td className="px-3 py-2 text-sm text-slate-600 dark:text-slate-400">{order.totalWeight?.toFixed(2) || 0}</td>
                    <td className="px-3 py-2 text-sm text-slate-600 dark:text-slate-400">{order.totalVolume?.toFixed(6) || 0}</td>
                ...
                <tr className="bg-slate-50 dark:bg-slate-800 font-semibold">
                  <td colSpan={2} className="px-3 py-3 text-sm text-slate-900 dark:text-white">ИТОГО:</td>
                  <td className="px-3 py-3 text-sm text-slate-900 dark:text-white">{consolidation.totalOrders}</td>
                  <td className="px-3 py-3 text-sm text-slate-900 dark:text-white">{consolidation.totalWeight.toFixed(2)}</td>
                  <td className="px-3 py-3 text-sm text-slate-900 dark:text-white">{consolidation.totalVolume.toFixed(6)}</td>
